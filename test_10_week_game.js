...
const context = vm.createContext({
  ...
  stagedDropdownHints: html.includes('class="step-hint-select"') &&
    html.includes('function stepChoiceData') &&
    html.includes('class="op-pick-select"'),
...
const summary = vm.runInContext(`(() => {
  ...
  const geometryTypes=['squarePerimeter','rectPerimeter','trianglePerimeter','squareArea','rectArea'];
  const badGeometryGates=geometryTypes.filter(type=>{
    const question=tenGenerate(type,5,3);
    return !question.operationPrompt ||
      question.firstOperationOnly!==true ||
      question.revealScratchAfterOperation!==true ||
      !(question.opFor || (Array.isArray(question.opSequence) && question.opSequence.length));
  });
  return {
    ...
    geometryGatesChecked:geometryTypes.length,
    badGeometryGates,
    stressFailures,
